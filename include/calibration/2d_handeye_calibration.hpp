...
struct rotCmp
{
    bool operator()(const std::pair<uint16_t, std::pair<Pose2d, Pose2d>> &pose_r, 
                                    const std::pair<uint16_t, std::pair<Pose2d, Pose2d>> &pose_l)
    {
        return (std::fabs(pose_l.second.first.yaw_) > std::fabs(pose_r.second.first.yaw_));   
    }
};
...
class HandEyeCalib2D
{
    private:
        ...
        // 对pose 按照质量进行排序  
        std::priority_queue<std::pair<uint16_t, std::pair<Pose2d, Pose2d>>, 
            std::vector<std::pair<uint16_t, std::pair<Pose2d, Pose2d>>>, rotCmp> priority_pose_;
        // pose 数据库中的序号  
        std::queue<std::pair<uint16_t, std::pair<Pose2d, Pose2d>>> new_pose_pair_;    
        std::vector<std::pair<Pose2d, Pose2d>> pose_storage_;   

        Eigen::MatrixXd Q_;    // 计算旋转外参时的矩阵  
        Eigen::Quaterniond ext_q_result_;  
        Eigen::Vector2d ext_t_result_;  
        bool calib_done_ = false; 
        Pose2d primary_sensor_accum_pose_, sub_sensor_accum_pose_;
    ...
    public:  
         ...
         * @return 本组运动是否合格   
         */
        bool AddPose(const  Pose2d& pose_primary, const  Pose2d& pose_sub)
        {        
            // 首先检查pose 
            ...
            if (pose_storage_.size() < N_POSE) 
            {
                ...
                std::cout<<"num > 300, remove top theta: "<< priority_pose_.top().second.first.yaw_<<std::endl;
            ...
            std::cout<<common::GREEN<<"pose_storage_.size(): "<< pose_storage_.size()<<common::RESET<<std::endl;
        ...
        bool CalibMethod1()
        {
            ...
            {
                const Pose2d &pose_primary = pose_storage_[i].first;
                const Pose2d &pose_sub = pose_storage_[i].second;
                ...
                A_k << pose_primary.x_, cos(pose_primary.yaw_) - 1, -sin(pose_primary.yaw_), -pose_sub.x_, pose_sub.y_,
                               pose_primary.y_, sin(pose_primary.yaw_),  cos(pose_primary.yaw_) - 1, -pose_sub.y_, -pose_sub.x_;
        ...
        bool CalibMethod2()
        {
            ...
            for (size_t i = 0; i < pose_storage_.size(); i++)
            {
                ...
                A_k << cos(pose_primary.yaw_) - 1, -sin(pose_primary.yaw_), -pose_sub.x_, pose_sub.y_,
                                sin(pose_primary.yaw_),  cos(pose_primary.yaw_) - 1, -pose_sub.y_, -pose_sub.x_;
                b_k << -pose_primary.x_, -pose_primary.y_; 
                M = M + A_k.transpose() * A_k; 
            ...
            {
                std::cout << common::RED<<"矩阵接近奇异，条件数： "<<cond << std::endl;
                return false;
            }
        ...
        bool GetCalibResult(Pose2d &result)
        {
            if (calib_done_)
            {
                result.x_ = ext_t_result_[0]; 
                result.y_ = ext_t_result_[1]; 
                ...
                result.yaw_ = eulerAngle[0];
                result.orientation_ = ext_q_result_;  
                return true;
            }
            return false;  
        }
    ...
    protected:
         ...
         * @return {*}
         */        
        Eigen::VectorXd SolvePenaltyFunction(int const& iterater, Eigen::MatrixXd const& H, Eigen::MatrixXd const& b)
        {
            float lamda = 0.1;
            float beta = 5;
            float thresh = 0.05; 
            ...
            for (int i = 0; i < iterater; i++)
            {
                ...
                {
                    std::cout<<common::GREEN<<"calib OK !!"<<std::endl;
                    return y;  
                }
                lamda *= beta;  
            }
        ...
        {   
            ...
            if ((pow(b,2) - 4 * a * c) >= 0)
            {
                ...
                std::cout <<common::RED<< "lamda 无零解, 问题无非零解！！！"<<common::RESET << std::endl;
         ...
         * @return {*}
         */            
        bool checkScrewMotion(const Pose2d &pose_primary, const Pose2d &pose_sub)
        {
            // 先检查当前数据是否正确
            //  检查旋转  
            //  刚性连接的传感器   旋转角度应该相同
            double r_dis = abs(pose_primary.yaw_ - pose_sub.yaw_);   
            // 表明误差太大  或 着本次运动的旋转太小
            //if ((r_dis > EPSILON_R) || abs(pose_primary.yaw_) < 0.01)
            if ((r_dis > EPSILON_R))
            {
                std::cout<<"pose_primary.yaw_: "<<pose_primary.yaw_<<" ,pose_sub.yaw_"
                <<pose_sub.yaw_<<std::endl;
            ...
            std::cout<<"pose_primary x: "<<pose_primary.x_<<", y: "<<pose_primary.y_
            <<", orientation_: "<<pose_primary.orientation_.coeffs().transpose()<<std::endl;
            std::cout<<"pose_sub x: "<<pose_sub.x_<<", y: "<<pose_sub.y_
            <<", orientation_: "<<pose_sub.orientation_.coeffs().transpose()<<std::endl;

            // 旋转累计到一定大小时
            if (std::fabs(primary_sensor_accum_pose_.yaw_)> ROT_THRESH 
                    || std::fabs(sub_sensor_accum_pose_.yaw_) > ROT_THRESH)
            {
                // 位移太大则  不考虑 
                if (primary_sensor_accum_pose_.x_ * primary_sensor_accum_pose_.x_
                    + primary_sensor_accum_pose_.y_ * primary_sensor_accum_pose_.y_ > 1)
                {
                    primary_sensor_accum_pose_.SetIdentity();
                    sub_sensor_accum_pose_.SetIdentity();  
                    std::cout<<common::YELLOW<<"move too long"<<std::endl;
                    return false;
                }

                std::cout<<"primary_sensor_accum_pose_ orientation_: "<<primary_sensor_accum_pose_.orientation_.coeffs().transpose()<<std::endl;
                                std::cout<<"sub_sensor_accum_pose_ orientation_: "<<sub_sensor_accum_pose_.orientation_.coeffs().transpose()<<std::endl;
                return true;  
            }
            return false;  
        }
