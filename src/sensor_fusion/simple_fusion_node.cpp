 ...
 * @FileName: 
 * @Author: lwh
 * @Description:  一种简易的融合实现：1、odom&imu 提供畸变去除以及位姿预测 
 *                                                                              2、采用最小二乘在线对IMUbias进行估计 
 */
...
class FusionOdometryNode {
    public:
        ...
        FusionOdometryNode() {
            ...
            undistorted_pointcloud_publisher_ = node_handle_.advertise<sensor_msgs::PointCloud2>(
                "undistorted_pointcloud", 1, this);

            org_pointcloud_publisher_ = node_handle_.advertise<sensor_msgs::PointCloud2>(
                "org_pointcloud", 1, this);

            org_laserscan_publisher_ = node_handle_.advertise<sensor_msgs::LaserScan>(
                "org_laserscan", 1, this);
        ...
        void ScanCallback(const sensor_msgs::LaserScan::ConstPtr &laserScanMsg) {
            ...
            if (!scan_init) {
                ...
                if (laser_frame == "") {
                    laser_frame = laserScanMsg->header.frame_id;
                }
            ...
            LaserPointCloud<PointT> laser_pointcloud;
            laser_pointcloud.range_.reserve(laserScanMsg->ranges.size());
            laser_pointcloud.index_.reserve(laserScanMsg->ranges.size());
            laser_pointcloud.pointcloud_.points.reserve(laserScanMsg->ranges.size());
            laser_pointcloud.time_stamp_ = laserScanMsg->header.stamp.toSec();
            // 极坐标转换为笛卡尔坐标
            ...
            for (int i = 0; i < laserScanMsg->ranges.size(); i++) {
                ...
                PointT point; 
                // 计算雷达数据的 x y 坐标
                point.x =  laserScanMsg->ranges[i] * laser_info_.index_cos_[i];
                point.y = laserScanMsg->ranges[i] * laser_info_.index_sin_[i];
                laser_pointcloud.pointcloud_.push_back(std::move(point)); 
                laser_pointcloud.range_.push_back(laserScanMsg->ranges[i]); 
                laser_pointcloud.index_.push_back(i); 
            }
            laser_buf_.emplace_back(std::move(laser_pointcloud));

            sensor_msgs::PointCloud2 cloud_msg;
            pcl::toROSMsg(laser_pointcloud.pointcloud_, cloud_msg);
            cloud_msg.header = laserScanMsg->header; 
            cloud_msg.header.stamp = ros::Time(laser_pointcloud.time_stamp_); 
            //cloud_msg.header.frame_id = laser_frame;
            org_pointcloud_publisher_.publish(cloud_msg);    // rviz在laser坐标系下显示正常  在odom系下不正常  

            //org_laserscan_publisher_.publish(*laserScanMsg);
            
            // sensor_msgs::PointCloud2 cloud_msg2;
            // pcl::toROSMsg(laser_pointcloud.pointcloud_, cloud_msg2);
            // // cloud_msg2.header.stamp = ros::Time(laser_pointcloud.time_stamp_); 
            // // cloud_msg2.header.frame_id = laser_frame;
            // cloud_msg2.header = laserScanMsg->header; 
            // cloud_msg2.header.stamp = ros::Time(laser_pointcloud.time_stamp_); 
            // undistorted_pointcloud_publisher_.publish(cloud_msg2);
        }

        // 去畸变
        void undistortPoints(LaserPointCloud<PointT> &curr_laser) {
            // 先获取激光起始点的odom位姿
            Pose2d start_pose; 
            if (!dead_reckon_.ExtrapolatorPose(curr_laser.time_stamp_, start_pose)) {
                std::cout<<"start_pose error"<<std::endl;
                return;  
            }
            // 遍历每个激光点 获取每个激光点相对于起始位置的变换 
            for (int i = 0; i < curr_laser.pointcloud_.size(); i++) {
                // 时间戳
                double time_stamp = curr_laser.time_stamp_
                    + curr_laser.index_[i] * laser_info_.time_increment_; 
                // 时间戳位置处的odom pose
                Pose2d curr_pose; 
               if (!dead_reckon_.ExtrapolatorPose(time_stamp, curr_pose)) {
                   std::cout<<"ExtrapolatorPose error"<<std::endl;
               }
                Pose2d pose_start_curr = start_pose.inv() * curr_pose; 
                // 如果外参没有初始化   那么仅仅使用旋转数据 
                if (!odom_ext_init) {
                    pose_start_curr.SetTransform(0, 0);
                }
                Eigen::Vector2d point_pos(curr_laser.pointcloud_.points[i].x, curr_laser.pointcloud_.points[i].y); 
                Eigen::Vector2d point_corrected_pos = pose_start_curr * point_pos; 
                curr_laser.pointcloud_.points[i].x = point_corrected_pos.x();
                curr_laser.pointcloud_.points[i].y = point_corrected_pos.y();
                curr_laser.range_[i] = point_corrected_pos.norm();  
            }
        }
        ...
        void Estimator() {
            while(1) {
                { 
                    ...
                    // 1、读取激光数据  2、进行畸变去除   3、在线标定odom与激光的外参  4、融合里程计运行实时估计bias 
                    if (laser_buf_.size() >= 2 ) {
                        LaserPointCloud<PointT>& curr_laser = laser_buf_.front();  
                        // odom对当前激光的pose进行预测
                        Pose2d predict_pose; 
                        // std::cout<<"pre ExtrapolatorPose"<<std::endl;
                        dead_reckon_.ExtrapolatorPose(curr_laser.time_stamp_, predict_pose);
                        // std::cout<<"predict_pose x: "<<predict_pose.GetX()<<",y: "<<predict_pose.GetY()
                        // <<", yaw: "<<predict_pose.GetYaw()<<std::endl;
                        // // 发布航迹推算的结果
                        // nav_msgs::Odometry odom_msg;
                        // odom_msg.header.stamp = ros::Time(curr_laser.time_stamp_); 
                        // odom_msg.header.frame_id = "odom"; 
                        // odom_msg.pose.pose.position.x = predict_pose.GetX();
                        // odom_msg.pose.pose.position.y = predict_pose.GetY();
                        // odom_msg.pose.pose.orientation = tf::createQuaternionMsgFromYaw(predict_pose.GetYaw());
                        // odom_pub.publish(odom_msg); 
                        // 去除畸变
                        undistortPoints(curr_laser);  
                        // std::cout<<"undistortPoints done"<<std::endl;
                        // 发布去除了畸变的激光 
                        PublicLaserPointcloud(curr_laser);  
                        laser_buf_.pop_front();
                    }
        ...
        void PublicLaserPointcloud(LaserPointCloud<PointT> const& curr_laser) {
            sensor_msgs::PointCloud2 cloud_msg;
            pcl::toROSMsg(curr_laser.pointcloud_, cloud_msg);
            cloud_msg.header.stamp = ros::Time(curr_laser.time_stamp_); 
            cloud_msg.header.frame_id = laser_frame;
            undistorted_pointcloud_publisher_.publish(cloud_msg);
        }
    ...
    private:
        ...
        struct LaserInfo {
            ...
            double time_increment_;  // 激光点的时间增量
        }laser_info_;
        ...
        ros::Publisher org_pointcloud_publisher_; 
        ros::Publisher org_laserscan_publisher_; 
        ...
        std::deque<LaserPointCloud<PointT>> laser_buf_;  
        ...
        bool odom_ext_init = false;  // odom的外参初始化
